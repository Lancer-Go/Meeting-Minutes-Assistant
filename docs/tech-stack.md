...
| --- | --- | --- | --- |
| 80min 会议（M0 实测 22,632 字） | 4.5 万~9 万（summary + extractor 各单次全量） | 5k~15k（纪要 + 结构化抽取） | ≈ **¥0.2~0.6/场（空闲）** |
| 高峰时段（×2） | 同上 | 同上 | ≈ **¥0.4~1.2/场** |

- 假设：1 汉字 ≈ 1~2 token；summary 与 extractor 各单次全量读入（不再分块）。若重复读入触发上下文缓存，输入成本趋近命中价，下限可低至 ¥0.1 量级。
- 对照：M0 实测（V3 时代）80min 会议 LLM 成本 ¥0.0453，与本推算同量级；**正式值待 M3「成本监控与限额告警」落地后重测**。
- 结论：**空闲时段跑批单场可稳在 ¥1 KPI 内**；高峰时段 2h 长会议可能接近/超过 ¥1/场。降本手段：① 空闲时段跑批（半价）；② 切换 **deepseek-v4-flash**（价格约为 Pro 的 1/3、并发上限更高，质量略低，灰度对比后作低成本通道 / M4 多模型切换备选）；③ 利用上下文缓存（summary 与 extractor 重复读入同一转写，天然命中缓存）。
