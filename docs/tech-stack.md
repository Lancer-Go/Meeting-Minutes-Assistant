...
> 说明：本文汇总技术选型（Part A）与技术设计（Part B：需求、架构、方案、数据、测试、部署）。标记 🔶【建议】为倾向方案，✅【已定】为已决项。关键决策（云端 SaaS 优先 / 走云 API / 中文为主 / ≤ 2 小时 / 利润 0 性价比优先）见 [mission.md §8](mission.md)，最终以 M0 实测数据锁定具体厂商。
...
| 方案 | 类型 | 优点 | 缺点 | 适用 |
| --- | --- | --- | --- | --- |
| **阿里云 / 讯飞 / 腾讯云 ASR** | 云API | 准确率高、省心、标点好、带说话人分离 | 按量收费、数据出域 | ✅ **云端 SaaS 首选** |
| Azure / Google / OpenAI whisper-api | 云API | 多语言、省心 | 外资云、成本、合规 | 对标外企 / 多语场景 |
| **Whisper / faster-whisper** | 开源 | 中英强、可离线、免费 | 需 GPU / 相对慢 | 后期本地化选项 |
...
| 决策点 | 已定方向 | M0 待锁定 |
| --- | --- | --- |
| ASR 主方案 | ✅ 云 API（接受数据出域） | 具体厂商（阿里云 / 讯飞 / 腾讯云），依据中文准确率、成本、说话人分离 |
| LLM 主方案 | ✅ 性价比高（利润 0） | 具体型号（DeepSeek / Qwen），依据纪要质量、成本 |
| 部署形态 | ✅ 云端 SaaS 优先 | 云平台选型（K8s / 云托管） |
...
| **asr** | 语音转文本、说话人分离、时间戳 | 云 ASR（阿里云 / 讯飞 / 腾讯云） |
| **nlp** | 文本清洗、分段、角色识别、关键词 | spaCy / 规则 + LLM |
| **summary** | LLM 纪要生成、模板化 | DeepSeek / Qwen |
...
**③ 语音转文字（ASR）**：抽象 `ASRProvider` 接口，首期接云 ASR；说话人分离优先用云 ASR 内置能力，pyannote / whisperx 作兜底。
