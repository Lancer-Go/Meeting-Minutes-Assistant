...
| M1 MVP | 3–4 周 | FR-01~07 | G1, G2, G4 | 单机可用的完整闭环 |
...
### 🎯 目标
用一段真实会议录音跑通「音频 → 转写 → 纪要」最小闭环，用实测数据锁定 ASR 与 LLM 选型，验证可行性与成本。

### 🔧 详细实现
- 输入一段 **≥ 10 分钟**的真实会议音频（普通话优先）。
- FFmpeg 抽取并标准化音轨（16kHz 单声道 WAV）。
- faster-whisper（或云 ASR）转写为带时间戳文本。
- 调用 LLM（DeepSeek / Qwen）输出结构化 Markdown 纪要。

### 🛠 实现方法
- **技术栈**：Python 3.11 + FFmpeg + faster-whisper + openai SDK（兼容 DeepSeek/Qwen 的 OpenAI 兼容接口）。
- **形态**：单脚本 `pipeline.py` 串联，**暂不做服务化**（不建 Web、不建队列）。
- **环境**：`venv` 隔离；faster-whisper 支持 CPU int8 跑 small 模型（无 GPU 也能跑通验证）。

### 📋 工作顺序
1. 环境准备：建 `venv`，安装 `ffmpeg`、`faster-whisper`、`openai`。
2. 准备样例：录制/获取一段真实会议音频（≥10 分钟，普通话）。
3. 写 `audio.py`：FFmpeg 抽音轨 + 标准化，验证输出 WAV 规格正确、可播放。
4. 写 `asr.py`：接入 faster-whisper，验证转写文本完整、含时间戳。
...
- 明确结论：锁定 ASR 主方案与 LLM 主方案（供 M1 使用）。
...
### 🎯 目标
交付单机可用的完整闭环，满足 FR-01~07，无技术背景用户可自助"上传 → 得到纪要"。

### 🔧 详细实现
- **文件上传**：支持视频/音频（MP4 / MKV / WAV / MP3 / M4A），格式与大小校验。
- **任务管理**：创建任务、状态机（pending / running / succeeded / failed）、进度展示。
...
### ✅ 验证成果
**可交付物**：可单机运行的服务（Docker Compose 一键启动）。
...
- **说话人分离**：接入 pyannote / whisperx（diarization），转写文本带 speaker 标注。
- **角色识别**：规则 + LLM 辅助判断角色。
- **模板引擎**：Jinja2 渲染多种模板。
...
> 📌 **下一步建议**：立即启动 M0 —— 准备一份真实会议录音，跑通「FFmpeg + Whisper + LLM」最小闭环，用实测数据锁定选型。
