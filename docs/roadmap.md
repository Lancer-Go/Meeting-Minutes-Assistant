...
> 📌 **当前进度**：M0（选型锁定）与 M1（MVP 闭环）已完成；下一步推进 **M2 · 结构增强**（行动项 / 说话人 / 模板 / 编辑检索）。
>
> 🔄 变更（2026-08-25）：LLM 主方案由 DeepSeek-V3 升级为 DeepSeek-V4 Pro（deepseek-v4-pro）；转写阶段新增按切片推进的进度展示（「第 x/N 段已完成」）。
