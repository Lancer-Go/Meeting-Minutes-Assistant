...
> 本文把 roadmap.md 中 M1 的 8 步工作顺序组织为可执行的**任务组**。每个任务组含目标、任务项、产出与验收；任务组之间按依赖推进（前一组的验收是后一组的输入）。M1 复用 M0 已锁定选型（ASR 腾讯云 16k_zh / LLM DeepSeek-chat），从脚本式 `pipeline.py` 升级为 FastAPI 服务 + 异步全链路。
...
- **任务项**：
  - 拆出 `audio`（FFmpeg 抽音轨/归一化）、`asr`（`ASRProvider` 抽象，主用腾讯云 16k_zh）、`summary`（`LLMProvider` 抽象，主用 DeepSeek-chat）、`render`（Markdown 渲染）。
...
- **任务项**：
  ...
  - 异步执行 `audio → asr → summary → render` 全链路，逐段更新 `progress`。
...
- **任务项**：
  ...
  - 进度展示（轮询 `GET /api/tasks/{id}` 状态与进度）。
  - 结果下载（完成后提供纪要下载链接）。
- **产出**：可用的上传页 + 进度条 + 下载。
